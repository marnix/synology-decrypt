...
def test_decrypt_enc_key1_v1():
    ...
    assert (
        core.decrypted_with_password(enc_key1_binary, PASSWORD, salt=b"")
        == b"BxY2A-ouRpI8YRvmiWii5KkCF3LVN1O6"
    )
...
def test_decrypt_enc_key1_v3():
    ...
    enc_key1_binary = base64.b64decode(enc_key1)
    assert (
        core.decrypted_with_password(enc_key1_binary, PASSWORD, salt=salt)
        == b"EA23EB5F36B9008AC73498A8FC53884D1D7EFBA052F902F44B44D40409CBC215"
    )
...
def test_decrypt_enc_key2_v1():
    ...
    assert (
        core.decrypted_with_private_key(enc_key2_binary, PRIVATE_KEY)
        == b"BxY2A-ouRpI8YRvmiWii5KkCF3LVN1O6"
    )
...
def test_decode_single_line_file_v1():
    with open("tests/testfiles-v1/csenc/single-line.txt", "rb") as f:
        ...
        assert (
            decrypted_uncompressed_data == b"Just a single line, no newline character at the end..."
        )
...
def test_decode_single_line_file_v3():
    with open("tests/testfiles-v3/csenc/ssingle-line.txt", "rb") as f:
        ...
        assert next(s) == (
            "enc_key1",
            "6Gbow/t0ltbdXw2L79IS41HTVY7ffwl7vlmUs4CCOmtoqSIdxDxTcmt2cmjE38AfkvmTg0BwcK5WIsEMJwA81NS8vaHUv74D9XhPXMRclbM=",
        )
        assert next(s) == (
            "enc_key2",
        ...
        decrypted_compressed_data = core.decrypted_with_password(
            data, binascii.unhexlify(session_key), salt=b""
        )
        decrypted_uncompressed_data = lz4_uncompress(decrypted_compressed_data)
        assert (
            decrypted_uncompressed_data == b"Just a single line, no newline character at the end..."
