...
def decrypt_stream(instream, outstream, password=None, private_key=None):
        ...
        for (key, value) in decode_csenc_stream(instream):
            for case in switch(key):
                if case("digest"):
                    if value != "md5":
                        LOGGER.warning(
                            'found unexpected digest "%s": cannot verify checksum', value
                        )
                ...
                if case("key1_hash"):
                    if password != None:
                        ...
                        if value != actual_password_hash:
                            LOGGER.warning(
                                "found key1_hash %s but expected %s", actual_password_hash, value
                            )
                    break
                if case("key2_hash"):
                ...
                if case("version"):
                    ...
                    expected_version_numbers = [
                        ...
                        OrderedDict([("major", 3), ("minor", 0)]),
                        OrderedDict([("major", 3), ("minor", 1)]),
                    ]
                    if version not in expected_version_numbers:
                        raise Exception(
                            ...
                            + str(expected_version_numbers)
                        )
                    if (version["major"] > 1) != (salt != b""):
                        version_string = "%d.%d" % (version["major"], version["minor"])
                        LOGGER.warning(
                            "salt is expected in version 3+ (version: %s, salt present: %s)",
                            version_string,
                            (salt != b""),
                        )
                    break
                if case(None):
                    if decryptor == None:
                        ...
                        if session_key == None:
                            raise Exception(
                                "not enough information to decrypt data: need either password and enc_key1 or private key and enc_key2"
                            )
                        if session_key_hash == None:
                            LOGGER.warning(
                                "did not find session_key_hash to verify the session key"
                            )
                        else:
                            actual_session_key_hash = salted_hash_of(
                                session_key_hash[:10], session_key
                            )
                            if session_key_hash != actual_session_key_hash:
                                LOGGER.warning(
                                    "found session_key_hash %s but expected %s",
                                    actual_session_key_hash,
                                    session_key_hash,
                                )
                        decryptor = decryptor_with_password(
                            binascii.unhexlify(session_key) if salt else session_key, salt=b""
                        )
                    if decrypted_chunk:
    ...
    if decrypt_stream.md5_digestor != None and expected_md5_digest != None:
        ...
        if actual_md5_digest != expected_md5_digest:
            raise Exception(
                "expected md5 digest %s but found %s", expected_md5_digest, actual_md5_digest
            )
